...
| `try` / `catch` / `finally` | Effect | Yes | Partial | `codegen/stmt.ts` + `runtime_exception.h` | `test/effects/exceptions.test.ts` | `try/catch` works in the narrow model and straight-line `finally` now works too. Remaining gaps are control-transfer corners inside protected blocks and richer typed errors. |
