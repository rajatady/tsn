...
The default UI verification path now includes the native gallery app at [conformance/gallery.tsx](/Users/kumardivyarajat/.codex/worktrees/94d7/vite/conformance/gallery.tsx). `bash harness/ui-conformance.sh` builds it, launches it, drives it through the inspector, and writes screenshots plus tree dumps to `/tmp/tsn-ui-conformance`.
