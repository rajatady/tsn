...
TSX/JSX is not supported in the simplified compiler. `.tsx` inputs fail fast, and JSX syntax inside `.ts` files is rejected explicitly.
...
- Resolution tries: `.ts` and `/index.ts`
